...
async def editar_video(
    ...
    """
    Aplica edições ao vídeo:
    - watermark_text: texto de marca d'água (ex: @usuario)
    - caption_text:   legenda fixa na parte inferior
    - crop_start:     cortar a partir de N segundos
    - crop_end:       cortar até N segundos (0 = sem corte)
    """
    _auth(x_api_secret)
    ...
    logger.info(f"[editar] job={job_id} cmd={' '.join(cmd)}")
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)

    if result.returncode != 0:
        logger.error(f"[editar] ffmpeg erro: {result.stderr[-500:]}")
        raise HTTPException(status_code=500, detail=f"FFmpeg falhou: {result.stderr[-300:]}")
